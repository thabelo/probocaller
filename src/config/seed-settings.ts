...
export const DEFAULT_SETTINGS: Array<{ key: string; value: string; description: string }> = [
  ...
  { key: 'SURVEY_RESULTS_MIN_CELL', value: '5', description: 'Smallest group of respondents a survey result may show exactly. An option chosen by fewer than this is held back rather than counted. Clamped one-way in code: raising this makes the rule stricter, lowering it below 5 has no effect.' },
  { key: 'SURVEY_RESULTS_RELEASE_THRESHOLD', value: '10', description: 'How many answers a survey needs before it reports anything at all, and the smallest audience it may be published to. Clamped one-way in code: raise it to be stricter, lowering it below 10 has no effect.' },
  { key: 'SURVEY_RESULTS_BATCH', value: '10', description: 'Answers are released to a business in whole batches of this size, so no single response ever moves a published number. Clamped one-way in code: raise it to be stricter, lowering it below 10 has no effect.' },
  { key: 'SURVEY_REPEAT_WINDOW_DAYS', value: '30', description: 'How far back to look when counting how often one business has already read a person\u2019s survey answers.' },
  { key: 'SURVEY_MAX_PER_RESPONDENT', value: '3', description: 'How many surveys one BUSINESS may read a given person\u2019s answers from inside the window before that person stops being offered its surveys. Per business, not across the platform \u2014 someone who installed Surveys to earn keeps earning from everyone else. Set to 0 to switch the limit off.' },
  { key: 'PROFILE_STALE_AFTER_DAYS', value: '90', description: 'How long a profile may go untouched before we ask whether anything has changed. A stale profile is matched to fewer surveys and worse offers. Set to 0 to stop asking entirely.' },
  { key: 'PROFILE_NUDGE_COOLDOWN_DAYS', value: '30', description: 'Never ask the same person again inside this many days. This is the difference between a nudge and nagging.' },
  { key: 'PROFILE_NUDGE_BATCH', value: '200', description: 'Most people to ask in one daily sweep, so a backlog is worked through gradually rather than messaging everybody at once the first time it runs.' },
