...
import { seedSettings } from '../config/seed-settings';
...
export class AdminService {
  ...
  async addCredit(id: number, amount: number) {
    ...
    await this.transactionService.log(
      ...
      amount,
      `Admin ${amount > 0 ? 'credit' : 'debit'} of R${Math.abs(amount).toFixed(2)}`,
    );
  ...
  /**
   * Bootstrap-seed every settings row. Delegates to the shared seeder, which
   * ConfigModule ALSO runs (via SETTINGS_SEEDED) before any provider that
   * reads settings during instantiation — this call from
   * AppModule.onModuleInit is now a belt-and-braces repeat, not the only
   * chance a fresh database gets to be seeded.
   */
  async seedDefaultConfig(): Promise<void> {
    await seedSettings(this.settingRepository);
  }
